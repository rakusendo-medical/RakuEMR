...
export const EPICS: EpicMeta[] = [
  {
    ...
    description: '病棟内のベッド配置と患者状態（隔離・拘束・外出・外泊等）を一目で把握できる画面。病室移動・割り当て・カルテ画面への遷移の起点。',
  },
  {
    id: 'ep-02',
    title: '入退院手続き',
  ...
  },
  {
    ...
    status: 'completed',
    stories: [
      { id: 'us-53', label: 'オーダ入力（汎用フォーム）' },
      { id: 'us-54', label: '処方オーダ（処方／入院定時）' },
      { id: 'us-55', label: 'オーダ送信画面（起点）' },
      { id: 'us-56', label: '注射オーダ' },
      { id: 'us-57', label: '検査オーダ' },
      { id: 'us-58', label: 'ECT オーダ' },
      { id: 'us-59', label: '入院定時オーダ' },
      { id: 'us-60', label: 'IF オーダ' },
      { id: 'us-61', label: '治療形態（リハビリ）' },
      { id: 'us-62', label: 'テキストオーダ' },
      { id: 'us-63', label: '指示時カルテ記事作成＋カルテNo発行' },
      { id: 'us-64', label: '画像オーダ' },
      { id: 'us-65', label: '心理検査オーダ' },
    ],
    mainScreens: [
      { label: 'カルテ（オーダー入力・KarteActionBar から起動）', path: '/karte/P001', hint: 'アクションバー「オーダー入力」→ オーダ送信画面。全10種の入力ダイアログ' },
    ],
    description: 'カルテのアクションバー「オーダー入力」から起動するオーダ送信画面を起点に、OrderType 全10種（処方／注射／検査／画像／心理検査／ECT／入院定時／IF／テキスト＋治療形態＝リハビリ）を新規起票。指示時にカルテ記事作成・カルテNo発行を行う。',
  },
  {
    id: 'ep-12',
    title: '看護診断',
