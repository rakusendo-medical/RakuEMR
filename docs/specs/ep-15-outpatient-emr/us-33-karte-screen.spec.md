...
- [ ] **AC-10: タブ状態が URL ハッシュに反映される**
  ...
  - **Given** カルテ画面で診療録タブから別タブ（例: フローシート）に切り替えた
  - **When** ブラウザの戻るボタンをクリック
  - **Then** 診療録タブに戻る（タブ切替が履歴に積まれている）
  - **Note**: タブ切替（ユーザー操作）は履歴に積む（`navigate('#<hash>')` で `replace: false`、ブラウザバックで前タブに戻れる挙動）。一方、初期化時の URL 自動補正（無効ハッシュ → 既定タブ等の内部書換）は履歴に積まないため `replace: true` を維持する。ブラウザの戻る／進むでハッシュが変わった場合は `navigate` 呼び出しではなく `useEffect` で追従して再描画する
