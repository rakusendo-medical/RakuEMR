...
| --- | --- | --- | --- | --- |
| MASTER | マスターセッション | **ep-15 段階 1 / ep-16 段階 2 クローズ済**。**ep-17 段階 3 実装完了 → PM 目視待ち**（2026-05-11）。us-39 `/karte-alpha` 完全撤去（`4d1c8af`・ルート + `KarteAlphaPage.tsx` 1117 行削除 + 残 navigate 2 件 + MainLayout 条件削除 + screen-mapping/HANDOVER/epicData/us-16 spec 整合 + ソースコメント整理 + キャッチオール `*` 追加）。us-40 旧素材 + 死にコード 一括撤去（`97937f6`・`components/flowsheet/FlowsheetPage.tsx` + `components/karte/PatientHeader.tsx`/ActionBar/LifeTimeline/MedicalInfo/MedicalRecords 計 6 ファイル 1214 行削除）。tsc + vite build クリーン継続。us-41 リグレッション PM ブラウザ目視チェックリストは `docs/changes/ep-17-outpatient-emr-stage3.md` に整備済（ep-01〜10 / ep-15 / ep-16 / 撤去確認 計 30+ 項）。**最終形「カルテ画面 1 ファイル / 1 ルート」到達**。**次のアクション**: ① PM 目視結果反映（軽微 → 即時修正 / 深刻 → 新規 us 起票）／② us-41 完了後 ep-17 を ✅ 完了に移動 + HANDOVER 段階移行クローズ／③ S2 起票の死にコード対応 3 件（MASTER 待ち事項）／④ ep-12〜14 段階整理／⑤ worktree node_modules 共有方針（MASTER 待ち事項） | 段階 3 実装完了・PM 目視待ち | 2026-05-11 |
...
| 2026-05-11 | S2（PM 仕様提示） | **目安箱機能（新規 ep 候補・ep-18 想定）**: モック内データではなく **モック自体に対する意見・要望を集めるメタ機能**。PM 要求 8 項目: ① 配置はヘッダー（AppBar 右上）確定 ② 画面単位の改修要望（クリック時点 URL ベース自動登録）③ 改修への返事記載 ④ 日付管理 ⑤ 熱量管理（高 = 仕事で使えない / 中 = 別方法を考える必要 / 低 = 見た目・利便性）⑥ 匿名 or 記名（あだ名可）⑦ ログイン無しで共通管理（= 端末横断で同じデータを共有 = **backend 必須**）⑧ 登録時 Teams Webhook 送信。**技術スタック追加が必要**: 既存 React + Vite フロントエンド単独では完結しない。アーキテクチャ選択肢 案 A: 既存リポ内 `RakuEMR/server/`（Express + better-sqlite3 + `.env.local`）+ Vite proxy ／ 案 B: 別リポ feedback service 分離 ／ 案 C: サーバレス（Cloudflare D1 / Supabase 等）。**判断項目**: ① アーキテクチャ案 A/B/C 選択 ② SQLite スキーマ設計（feedback / replies テーブル）③ env variables 命名（`VITE_FEEDBACK_API_BASE` / `FEEDBACK_DB_PATH` / `TEAMS_WEBHOOK_URL` 等）④ `scripts/setup-worktrees.sh` への backend 起動手順追加要否 ⑤ 担当ワーカーアサイン ⑥ PM の残り仕様確認（画面論理名の自動辞書 / 匿名 ID の持続化 / 熱量 UI / 返信権限 / Webhook 失敗時挙動 / フィルタ UI） | 新規（`server/` ディレクトリ / `package.json` 依存追加 / `.env.local` / `scripts/setup-worktrees.sh` / `vite.config.ts` proxy 設定 / `docs/issues/epics/ep-18-feedback-box.md` 新規 / `docs/specs/ep-18-feedback-box/` 新規 / `src/components/feedback/` 新規 / `src/layouts/MainLayout.tsx`（AppBar 改修）） | PM と MASTER で直接（S2 セッション外） |
...
| ep-17 外来 EMR 刷新・段階 3 | 外来・共通 | us-39, us-40, us-41 | us-39 (`4d1c8af`) / us-40 (`97937f6`) 実装完了。us-41 リグレッション PM ブラウザ目視待ち（チェックリストは `docs/changes/ep-17-outpatient-emr-stage3.md`） |
