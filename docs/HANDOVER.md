...
| S3 | ワーカー | **ep-16 段階 2 us-50「指示簿タブ実装（最小実装）」着手**。spec [`us-50-orders-tab.spec.md`](specs/ep-16-outpatient-emr-stage2/us-50-orders-tab.spec.md) を起点に `src/components/karte/OrdersTab.tsx`（新規）と `src/components/karte/KartePage.tsx`（`KarteTabContent` の `orders` 分岐 + meta プレースホルダ削除）を編集予定。`mockData.ts` の既存 `ORDERS` を `patientId` でフィルタした read-only 一覧（type Chip 8 / status Chip 6 / 件数 / 0 件空状態 / 「指示状況タブを開く」ボタン → `commitTab('order-status')`）。`types`/`store`/`mockData.MASTER_*`/`common` 触らず。S4 の us-51 と並行（`KartePage.tsx` の `KarteTabContent` 関数内が編集重複点・短時間で閉じる方針）。検証は tsc + build、ブラウザ目視は MASTER 統合確認時に依頼予定 | 着手 | 2026-05-07 |
