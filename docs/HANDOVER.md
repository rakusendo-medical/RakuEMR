...
| --- | --- | --- | --- | --- |
| MASTER | マスターセッション | **ep-15 / ep-16 / ep-17 すべてクローズ済**（段階 1〜3 完走・2026-05-11）。**最終形「カルテ画面 1 ファイル / 1 ルート」到達**: `/karte/:patientId` + `KartePage.tsx` に収束、旧 `KarteAlphaPage.tsx` + 旧素材 6 ファイル（計 2331 行）撤去。本日内訳: ep-16 closure（us-36 サブ C ep-10 流用 NursingRecordDialog 流用 / 用語「看護ケア」全廃 / サイドバー看護整理は S2 から取込）／ep-17 us-39（`/karte-alpha` 撤去）/us-40（旧素材撤去）/us-41（PM 目視チェックリスト整備、実施は任意）。+ /admission UX 修正 4 件: 「新規入退院指示」タブ削除 / 「移動歴」ボタン削除 / 凡例カレンダー右上化 + セル配色統一 + 状態依存挙動の注釈 / モック切替バーに注釈。隔離拘束一覧の行クリック → カルテ遷移化。エピック評価から ep-15/16/17 除外。ep-18 目安箱は別システム実装方針で本リポ対象外（クローズ）。**次のアクション**: ① S2 起票の死にコード対応 3 件（孤立 4 / unused 53 / `/nursing`系ルート温存可否）の判断 ② ep-12〜14 段階整理 ③ worktree node_modules 共有方針（MASTER 待ち事項） | 段階 1〜3 完走・全クローズ | 2026-05-11 |
...
| S2 | ワーカー | **ep-16 段階 2 us-36「入院アクション本実装」サブ A・サブ B 完了**。**サブ A**（2026-05-07・commit `b17e912`）: 設計合議で案 2「2 ボタン分割（KarteAlphaPage 同パターン）」採択。spec / changes / 実装すべて更新済（詳細は `docs/changes/ep-16-outpatient-emr-stage2.md`「## us-36 サブ A 完了メモ」）。**サブ B**（本コミット）: ① `KarteActionBar.tsx` の `isolation-order` を活性化（disabled / Tooltip 削除）／② `KartePage.tsx` に `RestraintOrderDialog` を import、`restraintDialog` state（`{open, title, editId?}`）+ `openRestraintDialog` / `closeRestraintDialog` ハンドラ、`handleAction('isolation-order')` 分岐（既定タイトル「隔離開始」で起動）、`KarteTabContent` に `medical-record` 分岐 + `onRequestRestraintOrder` prop 追加、レンダー末尾に `<RestraintOrderDialog>` 追加／③ 新規 `MedicalRecordTab.tsx`（`SectionHeader.rightSlot` に既存 `RestraintOrderLinks` を埋込、mode='inpatient' 時のみ。本体は段階 3 / ep-17 移植までの placeholder）／④ `screen-mapping.tsv` に MedicalRecordTab.tsx 行追加。**事前想定の合議 2〜3 回は API 変更不要・既存コンポーネント再利用・スタブ実装で全て解消**（既存 `RestraintOrderDialog` / `RestraintOrderLinks` API 無変更、`SectionHeader` 無変更）。AC-B1〜B4 + AC-X1/X2 全達成、tsc / build クリーン、`types`/`store`/`mockData`/`common`/既存ダイアログ API 変更なし。**サブ C（看護ケア記録）は要件未確定 → PM + MASTER に実装方針 3 案（新規ダイアログ / ep-10 流用 / us-37 統合）からの選択を依頼予定**。ブラウザ目視は MASTER 段階 2 統合確認時に依頼 **2026-05-11 再起動**: worktree `~/project/RakuEMR-s2/` で再起動完了。`origin/main` を ff-merge（一晩で us-43 診療録タブ集約タイムライン・us-44 患者ヘッダー強化・us-46 診療情報パネル・us-50 指示簿・us-51 指示状況・エピック評価機能が main に取込済）。S2 のサブ A・B も merge 済（`cb9f5a1`）。`node_modules` は前回張った MASTER worktree への symlink を維持。**サブ C 着手**: ① マニュアル調査で「看護ケア記録」が参考システムに無いと判明 → PM 判断で **用語を「看護記録」に統一・廃止**／② 実装方針は **案 b（ep-10 既存 `NursingRecordDialog` 流用）** 採用／③「看護経過記録」は別 us で整理。**実装**: `KarteActionBar.tsx` の `nursing-care` を id `nursing-record`/label「看護記録」に rename + 活性化、`KartePage.tsx` に `NursingRecordDialog` を import + state + handler + 末尾レンダー（既存 API 無変更）。spec us-36 / _epic / us-35 / changes/ep-16 で「看護ケア記録」→「看護記録」一括置換、spec AC-C1〜C4 確定追記、changes 決定事項 #6 追加、サブ C 完了メモ追記。**us-36 全サブタスク完了**（A/B/C + X1/X2 全 AC 達成）。tsc / vite build クリーン、共有ファイル変更なし。ブラウザ目視は MASTER 段階 2 統合確認時に依頼。ep-15 系 spec / changes の用語修正は履歴として残置（MASTER が後日判断）。**フォローアップ（同 2026-05-11・PM 追加指示）**: ① 画面上の「看護ケア」全廃 → MainLayout サイドバー「看護ケア予定」→「看護予定」リネーム、NursingCarePlan body text、mockData の `actionButtons` 配列要素削除、KarteAlphaPage の `ACTION_BUTTONS` 配列要素削除（NoteAdd icon import も削除）、types/mockData のコメント修正／② **サイドバー看護セクション整理** → サイドバーから「看護記録」エントリ（`/nursing` 行き）と「看護予定」エントリ（`/nursing-care` 行き）を **両方削除**、看護記録系の入口は「部門記録簿」（`/nursing/records`）に一本化。`/nursing` / `/nursing-care` ルート自体は URL 直打ち用に温存／③ `Description` / `Favorite` icon import を未使用化に伴い削除。tsc / vite build クリーン。死にコード調査結果（孤立ファイル 4 件 + unused imports/locals 53 件 + /nursing 系ルート温存可否）は PM 指示で **MASTER 待ち事項に 3 行起票済**（L53-55 / 2026-05-11） | 完了（us-36 全クローズ可・MASTER 段階 2 統合確認 + 起票 3 件の判断待ち） | 2026-05-11 |
| S2 | ワーカー | **新タスク受領 (briefings/s2.md・MASTER 2026-05-29 更新)**: Flowsheet 単一 Table 化（B 案リファクタ）。対象 `src/components/flowsheet/Flowsheet.tsx`（747 行）の 7 つの独立 `<TableContainer>` + `<Table>` を **1 つの `<Table sx={{ tableLayout: 'fixed' }}>` + `<colgroup>`**（9 列: label 130px / sub 40px / day 110px×7）に統合。バイタルチャート部も `<TableRow>` + `<TableCell colSpan={9}>` に内包。背景: `/karte/:patientId#flowsheet` で 5/15・5/16 のデータ行が右にずれる課題（特定セル content が minWidth を破壊）。AC 7 件（統合・縦位置一致・ずれ解消・チャート列一致・tsc・build・表示変更なし）。触らない: 共有ファイル / `features/flowsheet/` / モックデータ中身。**2026-05-29 起動**: worktree `~/project/RakuEMR-s2/` で再起動、`origin/main` ff-merge（main 大幅進展取込・ep-15/16/17 全クローズ・KarteAlpha 撤去・ep-18 別システム化決定済）、`node_modules` symlink 維持。タスク把握完了、PM 着手 GO 待ち | 完了（Flowsheet 単一 Table 化・PM「見た目変えない」指示反映済・ブラウザ目視 MASTER 依頼） | 2026-05-29 |
...
| 2026-05-11 | S2（us-36 完了後の死にコード調査） | **孤立ファイル 4 件の削除可否**: `grep -rn "from.*<filename>" src` で誰からも import されないと確認したファイル ① `src/components/karte/MedicalRecords.tsx` ② `src/components/karte/MedicalInfo.tsx` ③ `src/components/karte/LifeTimeline.tsx`（us-45 生活歴タイムラインで再利用される可能性あり） ④ `src/components/flowsheet/FlowsheetPage.tsx`（旧 flowsheet、新 `src/features/flowsheet/pages/FlowsheetPage.tsx` に置換済）。④ を消すと連鎖的に ⑤ `src/components/karte/ActionBar.tsx` ⑥ `src/components/karte/PatientHeader.tsx`（④ のみが import 元）も削除可能。**判断項目**: ③ は us-45 着手時の再利用候補なので削除/温存どちらか／④⑤⑥ は段階 3 撤去対象の前準備として削除可能 | `src/components/karte/{MedicalRecords,MedicalInfo,LifeTimeline,ActionBar,PatientHeader}.tsx` / `src/components/flowsheet/FlowsheetPage.tsx` | us-45 着手判断時 or 段階 3 着手前 |
| 2026-05-11 | S2（us-36 完了後の死にコード調査） | **unused imports / locals 53 件の方針**: `tsc --noEmit --noUnusedLocals --noUnusedParameters` で 53 件検出。分布: karteAlpha/KarteAlphaPage 9（段階 3 で全廃）/ components/flowsheet/Flowsheet 7（旧）/ outing/OutingManagement 7（未実装ダイアログ痕跡）/ MainLayout 4（サイドバー整理の名残）/ isolation/IsolationRestraint 4 / その他 14 ファイル 22 件。**判断項目**: ① 個別修正をワーカーにタスクとして配るか／② `tsconfig.json` の `noUnusedLocals` / `noUnusedParameters` を `true` に切替（現状 false）して CI で恒久的に検出する／③ 全件 MASTER 一括処理／④ 一旦放置で段階 3 撤去時に消える分（karteAlpha 等）を待つ | `tsconfig.json` / 各 src ファイル | 段階 2 末 or 段階 3 着手前（緊急性低） |
| 2026-05-11 | S2（PM 指示・サイドバー整理時） | **`/nursing` と `/nursing-care` のルート温存可否**: PM 指示でサイドバーから「看護記録」「看護予定」エントリを削除したが、`src/routes/index.tsx` の `/nursing` （`NursingRecordView` 行き）と `/nursing-care`（`NursingCarePlan` 行き）のルート定義は URL 直打ち用に **温存**。**判断項目**: ① ルートも削除して `NursingRecordView` / `NursingCarePlan` コンポーネントも撤去するか／② 現状の温存を維持して URL 直打ちでアクセス可能なまま保つか／③ ルートだけ残してコンポーネントを placeholder に置換するか。`NursingCareSchedule` 型 / `NURSING_CARE_SCHEDULES` 定数は画面外識別子のため触らず温存中（PM 用語整理の対象外と判断） | `src/routes/index.tsx` / `src/components/nursing/NursingRecord.tsx` / `src/components/nursingCare/NursingCarePlan.tsx` / `src/types/index.ts`（`NursingCareSchedule`） / `src/data/mockData.ts`（`NURSING_CARE_SCHEDULES`） | 段階 3 着手前 or PM 追加判断時 |
| 2026-05-11 | S2（PM 仕様提示）→ クローズ | **目安箱機能（ep-18 想定）→ 別システム実装方針で本リポ対象外**（PM 判断 2026-05-11）。理由: ① backend 必須（端末横断データ共有 + Teams Webhook）で本リポの React + Vite モック範囲を逸脱／② 用途がモック自体のメタ評価で EMR 本体機能ではない／③ 別システム/別リポで実装する。本リポでは ep-18 を起票しない。当初提示の PM 要求 8 項目・アーキ案 A/B/C・SQLite スキーマ等のメモは `git log -p -- docs/HANDOVER.md` で参照可能 | （対象なし） | 終了 |
| 2026-06-30 | MASTER（e2e セルフレビュー） | **e2e ロケータ脆さフォローアップ**: [`e2e/karte.spec.ts:21`](../e2e/karte.spec.ts#L21) の `getByRole('tab', { name: 'フローシート' }).toBeVisible()` は現状 pass するが、`beforeEach` で診療録タブが active のためサブタブが未レンダー → 1件マッチで成立しているのみ。将来テスト文脈が変わると `フローシート・隔離拘束` メインタブ + フローシート画面内サブタブ `フローシート` の 2件マッチで strict-mode 違反になる。今回の karte:34 と同質。**対応案**: `getByRole('tab', { name: 'フローシート・隔離拘束' })` への置換（現状の他タブ visible 確認群と整合）。緊急性低（pass 中）。同種の脆さは他にもある可能性（[`e2e/care-plan.spec.ts`](../e2e/care-plan.spec.ts) の `text=/評価期限超過\|今月評価\|計画未立案/` 等は alternation で限定されており現状 OK）。 | `e2e/karte.spec.ts:21` | 次の e2e 整備タイミング or 他テスト追加時 |
...
| ep-16 外来 EMR 刷新（段階 2） | us-35〜38, us-43〜47, us-50〜52 | `/karte/:patientId`（入院 + 外来 両 mode）。段階 2 完了（2026-05-11 PM OK）。**α との機能等価以上を達成**（KarteAlphaPage の準備中タブが新カルテで実装済・8 ピクトグラム / ClinicalInfoPanel / 集約タイムライン / 編集ダイアログ等で進化）。前/次患者ナビは PM 判断で不採用。us-36 サブ A/B/C は S2 で全完了（2026-05-11） |
| ep-17 外来 EMR 刷新（段階 3） | us-39, us-40, us-41 | `/karte/:patientId` 1 ルート / `KartePage.tsx` 1 ファイルに収束。段階 3 完了（2026-05-11 PM OK）。`/karte-alpha` ルート + `KarteAlphaPage.tsx` 撤去 / 残 navigate 2 件統合 / 旧 karte 素材 5 件 + 死にコード `flowsheet/FlowsheetPage.tsx` 撤去（計 6 ファイル 1214 行）。PM ブラウザ目視は `docs/changes/ep-17-outpatient-emr-stage3.md` のチェックリスト整備済（実施タイミングは任意） |
...
- **S2 起票の死にコード対応 3 件**（MASTER 待ち事項参照）: ep-17 で旧 karte 系の死にコードは整理済だが、別領域に残るスコープ。次の整理対象として優先度判定する
