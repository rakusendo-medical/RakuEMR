...
| S2 | ワーカー | **ep-16 段階 2 us-36「入院アクション本実装」サブ A 完了**。**設計合議**: 案 2「2 ボタン分割（KarteAlphaPage 同パターン）」を PM が採択（2026-05-07）。spec を本決定に追従し AC-A1〜A4 / 振る舞い / 想定実装ステップを書き換え、changes 決定事項 #5 を追加。**実装**: ① `KarteActionBar.tsx` を 2 ボタン化（`buildInpatientActions(admissionState)` で 3 状態 × 2 ボタンの活性を動的計算、Tooltip で disabled 理由を表示）／② `KartePage.tsx` で `handleAction` に `admission-order` / `discharge-order` 分岐追加、レンダー末尾に既存 `<AdmissionOrderDialog>` / `<DischargeOrderDialog>` を `open` state で開閉（既存 API 無変更）。AC-A1〜A4 + AC-X1/X2 全達成、`tsc --noEmit` / `vite build` クリーン、共有ファイル無変更。**サブ B（隔離拘束指示）着手予定**（次コミット）。詳細は `docs/changes/ep-16-outpatient-emr-stage2.md`「## us-36 サブ A 入退院指示 完了メモ」。ブラウザ目視は MASTER 段階 2 統合確認時に依頼。**MASTER 申し送り**: setup-worktrees.sh の `node_modules` 共有が未対応のため検証時に MASTER の `node_modules` を symlink で借用（`package.json` / lockfile 一致確認済）。setup スクリプト改善は別途検討依頼 | サブ A 完了・サブ B 着手準備 | 2026-05-07 |
| S4 | ワーカー | **起動・待機中**。worktree 分離後の `~/project/RakuEMR-s4/`（`worker/s4`）で起動、`origin/main` 最新（`a40d0c3`）と一致でマージ不要、`.claude/briefings/` は未配備。前回担当 us-38（病棟マップ等の遷移元修正）は完了済（L20）、ep-16 段階 2 では us-37 が後続未アサイン（ep-12〜14 進捗ヒアリング待ち・MASTER 待ち事項参照）。次タスクの PM/MASTER 指示を待機 | 起動・待機中 | 2026-05-07 |
