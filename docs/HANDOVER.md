...
| --- | --- | --- | --- | --- |
| MASTER | マスターセッション | **ep-15 段階 1 クローズ済**（2026-05-06 PM OK 受領、commit `93bf3eb`）。**ブリーフィング機構導入**（commit `72424c8`）: `.claude/briefings/`（gitignore）に `common.md` 配置済、CLAUDE.md にロール起動時の参照ステップ追加。**ep-16 立ち上げ完了**: spec ディレクトリ `docs/specs/ep-16-outpatient-emr-stage2/` 新設、`_epic.md` + `us-35-inpatient-mode.spec.md` + `us-38-navigation-state-from.spec.md` を起こし。S3 / S4 向け briefing（`.claude/briefings/s3.md`・`s4.md`）も配置済。ep-15 us-34 spec に段階 2 申し送り（PM 判断 2 件: 預かり金非表示・メモ表示位置明示）を追記。**次のアクション**: ① S3 が us-35 に着手（briefing 通り）／② S4 が us-38 に並列着手（briefing 通り）／③ MASTER は Phase 0（KarteAlphaPage 機能インベントリ）を進めて us-36 着手準備（S2 アサイン用）／④ ep-12〜14 進捗を PM に確認して us-37 タイミング設計。**段階 1 干渉事例 2 件は履歴注記セクション参照** | ep-16 立ち上げ完了・S3/S4 着手指示済 | 2026-05-06 |
...
| S3 | ワーカー | **ep-16 段階 2 us-35「入院 mode 本実装」アサイン済**（着手指示のみ・実装未着手）。MASTER から ep-16 spec（`docs/specs/ep-16-outpatient-emr-stage2/_epic.md` + `us-35-inpatient-mode.spec.md`）と briefing（`.claude/briefings/s3.md`）を提供済。スコープ: ① mode='inpatient' のテーマ primary・看護過程タブ活性・「入院」Chip 本実装（段階 1 で枠あり → 補追／確認）／② 属性サブタブの「入院専用情報」セクション本実装（入院日・病棟・病室・受け持ち看護師の read-only 表示）／③ PM 判断 2 件の反映（預かり金セクション非表示・メモ表示位置ラベル付与）。**S4 の us-38 と並列着手可（編集域分離）**。共有ファイル変更見込みなし（types / store / mockData / common 触らず）、変更要請が出たら MASTER 待ち事項に起票 | 着手指示済・未着手 | 2026-05-06 |
| S4 | ワーカー | **ep-16 段階 2 us-38「病棟マップ等の遷移元修正」アサイン済**（着手指示のみ・実装未着手）。MASTER から spec（`us-38-navigation-state-from.spec.md`）と briefing（`.claude/briefings/s4.md`）を提供済。スコープ: ① 病棟マップ → `/karte/:patientId` + `state.from='ward-map'` 付与／② 入院患者一覧 → 同 `'patient-list'` 付与／③ `/karte-alpha/:patientId` ルートは段階 2 中は **温存**（段階 3 で撤去）。**S3 の us-35 と並列着手可**。修正対象は `wardMap/` と `patients/` 配下の `navigate` 呼び出しのみ（S3 担当の `karte/` と物理分離）。`KartePageLocationState` 型変更が必要になったら MASTER 待ち事項に起票（S3 と先行調整必要） | 着手指示済・未着手 | 2026-05-06 |
