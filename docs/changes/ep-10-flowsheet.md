...
## フローシート パターン変更（ver0.18・2026-07-06）

参考システムの「フローシート パターン変更」を、カルテ内フローシート（`src/components/flowsheet/Flowsheet.tsx`）に追加。

### 仕様

- **配置**: フローシートの**最下部**に「● パターン変更」欄を追加。**フローシート本体テーブルに行として統合**し、日付列（7 日）と縦に整列する（別テーブルにすると列がズレるため本体テーブル内に取り込む）。
  - ヘッダー行: 左に `[パターン変更]` ボタン、右に日付列（7 日）
  - **パターン適用で入力項目（子行）が展開される**（＝枠が増える）。各パターンはグループ見出し行（パターン名）＋その入力項目の行群で構成し、各項目 × 各日付セルを**日付枠ごとに新規入力**できる（空欄はプレースホルダ「新規作成」）。
  - **入力は適用開始日以降のみ**。ダイアログで登録した開始日より前の日付列は入力不可（薄グレーで区別）。
    - `ADL`（画像準拠・9 項目）: 更衣(20分) / 整容(20分) / 排泄動作(40分) / 食事準備(30分) / 昼食(40分) / 口腔ケア(10分) / ラジオ体操(10分) / 趣味活動(60分) / 身体機能訓練(40分)
    - `基本項目`: 睡眠時間 / 睡眠状態 / 排便 / 使用薬剤 / 清潔 / 寝具交換 / 面会
    - `体重`: 6時 / 11時 / 16時 / 20時 / 日差
    - `睡眠1` / `睡眠2` / `不穏時`: 各パターンの入力項目（モック）を定義（`PATTERN_ITEMS`）。未定義パターンはパターン名 1 項目にフォールバック
  - **未適用**: 名称なし・データ域は背景**グレー**（患者は「共通項目時間設定」が適用される）
- **`[パターン変更]` ボタン → ダイアログ「フローシートパターン変更」**
  - ヘッダー入力: **開始日**（date）＋ **パターン**（Select）＋ `☑ 複数パターン登録` `☐ 日の途中からの変更` ＋ `[登録]`
  - **パターン適用期間テーブル**: `パターン適用期間（開始日 〜）` ｜ `適用パターン` ｜ 行削除。下部に `[新規]`
  - フッター: `[履歴]`（モック）／ `[閉じる]`
  - `[登録]`／`[新規]` で開始日＋選択パターンを適用期間に追加。`[削除]` で除去。`[閉じる]` で確定（最下部バーに即反映）
- 適用パターン候補（モック）: 基本項目 / ADL / 睡眠1 / 睡眠2 / 不穏時 / 体重

### 実装

- `Flowsheet.tsx` 内に自己完結（`patternPeriods` state ＋ パターン変更ダイアログ）。`features/flowsheet/` の並行実装（PatternChangeDialog）には依存しない。
- 共有ファイル（`types` / `stores` / `mockData.MASTER_*` / `common`）変更なし。

### 検証

- `npx tsc --noEmit` クリーン
- e2e: `e2e/flowsheet.spec.ts` に「最下部 [パターン変更] → ダイアログ登録 → バー反映」を追加（flowsheet 31 件グリーン）
