...
## フローシート パターン変更（ver0.26・2026-07-31）

参考システムの「フローシートパターン変更」を、カルテ内フローシート（`src/components/flowsheet/Flowsheet.tsx`）に追加。パターン候補・入力項目はマスタ（`src/components/flowsheet/patternMaster.ts`）で管理する。

### 仕様

- **パターンボックス**: フローシートサブタブの最下部に配置し、フローシート本体テーブルに行として統合（日付列と縦に整列）。
  - 表示モードを **「適用パターン名」／「全パターン」** で切り替えられる。全パターン表示では未適用のパターンも薄グレーで並び、入力導線は出ない。
  - **適用期間テーブル**（開始日・パターン名）で適用中のパターンを管理する。
- **パターン適用と入力**
  - パターンを適用すると、そのパターンの入力項目（ケアメニュー）が子行として展開される。
  - 値の登録は **パターン見出しの入力ダイアログ経由のみ**（日付＋各項目をまとめて入力）。**日付セル自体は読み取り専用**とし、セル直接入力は行わない。
  - 入力できるのは **適用開始日以降** のみ。終了日を設定した場合は **終了日以降も入力不可**（ダイアログの日付は開始日〜終了日に制約）。
  - 日付ヘッダーのフローシートアイコンからは、その日に適用中のパターンのみを対象に入力ダイアログを開く（同日複数適用時は開始日が最も新しいものを対象）。
- **パターン変更（不可逆）**
  - 開始日・パターンの変更時は確認ダイアログを表示し、了承すると **当該期間の開始日以降のケアメニュー入力を削除**する。
- **削除ガード・表示制御**
  - 入力済みの値があるパターンは削除不可。
  - 隔離拘束サブタブではパターン変更セクションを表示しない。

### 実装

- パターン候補・ケア項目は `patternMaster.ts`（`FLOWSHEET_PATTERNS` / `PATTERN_CARE_ITEMS` / `PATTERN_OPTIONS` / `patternItems` / `careItemLabel`）に分離。`Flowsheet.tsx` はこれを参照する。
- 適用期間・入力値は `Flowsheet.tsx` 内の state で保持（モックのためセッション限定＝リロードで戻る）。
- 共有ファイル（`types` / `stores` / `mockData.MASTER_*` / `common`）の変更なし。
...
## サンプルデータの基準日統一と直近30日の実データ化（ver0.39・2026-08-25）

画面によって 2〜5 月とばらばらだったサンプルデータの日付を、基準日 **2026-08-24（月）** に統一。あわせて主要患者の直近 30 日分を実データ化した。

### 背景

「当日」の定義が feature ごとに独立して 5 系統存在し、画面間で数か月ずれたデータが表示されていた。

| 旧基準日 | 定義場所 | 影響範囲 |
| --- | --- | --- |
| 2026-02-24 | `generateVitalSigns`／`NURSING_RECORDS` | 旧カルテ系バイタル・看護記録 |
| 2026-03-10 | `MedicalRecordTab` の `MOCK_RECORDS` | 診療録タイムライン |
| 2026-04-21 | `carePlan/mockData.ts` の `TODAY` | 看護過程 |
| 2026-05-02 | `features/flowsheet/mockData.ts` の `TODAY` | 部門記録簿・一括入力 |
| 2026-05-19 | `orders/orderDate.ts` の `MOCK_TODAY` | オーダー・フローシート 7 日グリッド |

### 対応

- **基準日の単一ソースを新設**: `src/data/mockToday.ts`（`MOCK_TODAY = '2026-08-24'`）。`orderDate.ts` は再エクスポート、`carePlan`／`features/flowsheet` の `TODAY` はこれを参照する形に変更
- **日付は系統ごとのオフセットで平行移動**（+97／+114／+125／+167／+181 日）。在院日数・適用期間・記録の前後関係といった**相対関係は完全に維持**される
- **オフセットは原則 7 の倍数**とし曜日を保存（ハードコードされた `dayOfWeek` や週末表示を直さずに済む）。7 の倍数にできないのは部門記録簿系（+115）のみで、同系統に曜日ハードコードは無い
- **除外**: `src/data/revisions.ts`（改定履歴）とコード内コメントの日付は実日付のため対象外
- 手当てした特殊ケース: `generateVitalSigns` の `new Date(2026, 1, 24 …)` → `new Date(2026, 7, 25 …)`、隔離拘束一覧の月選択初期値 `'2026-06'` → `'2026-08'`

### 直近 30 日の実データ化

- `src/data/recentRecords.ts` を新設。P001〜P003 について **医師記録**（定期回診の曜日中心）・**看護記録**（日勤は毎日／夜勤は隔日）・**部門診療録**（作業療法・心理・PSW の実施曜日）を直近 30 日分、**乱数を使わない決定的な組み立て**で生成する（リロードで内容が変わらない）
- `RecordSummaryStrip` は実データを持つ患者では擬似生成を行わず実データを参照。対象外の患者は従来どおり `pseudoRate` による擬似生成にフォールバック
- `MedicalRecordTab` も実データを持つ患者では `MOCK_RECORDS`（全患者共通サンプル）に代えて当該患者の記録を並べる。部門診療録は本タブに対応カテゴリが無いため取り込まない
- 日付を再度動かす場合は `MOCK_TODAY` を変えるだけで本データは追従する（固定日付を持たない）

### 検証

- `npx tsc --noEmit` クリーン / `npm run build` クリーン
- E2E: 固定日付を参照する 6 ファイル（79 箇所）を同じオフセットで更新し、全スイートを実行
