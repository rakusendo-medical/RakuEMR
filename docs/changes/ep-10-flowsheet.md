...
> **⚠️ 注記（2026-07-31）**: 上表の「実装後 AC」は、当時の実装先である `FlowsheetPage`（`/flowsheet/:patientId`）上での自己申告値。
> その後 `/flowsheet/:patientId` はカルテのフローシートタブへ恒久リダイレクトされ、`FlowsheetPage` は到達不可となったため、
> **現行画面での実態とは一致しない**（特に us-17〜21）。現行画面ベースの点検結果は
> [`docs/specs/AUDIT-ep-10-2026-07-31.md`](../specs/AUDIT-ep-10-2026-07-31.md) を参照。
