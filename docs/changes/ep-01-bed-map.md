...
- ~~`入退院情報` ボタンを追加 → 表示病棟の入退院情報サマリを開く（中身はモック）~~
  - **【ver0.16・2026-07-06 廃止】** PM より「`入退院情報` ボタンは廃止する」との連絡。右サイドバー再設計（ver0.15）以降このボタンは導線が外れ実質オーファンだったため、ダイアログごと廃止した。
  - ボタンで開いていた入退院情報サマリ（在院患者・稼働率・稼働ベッド・隔離中・拘束中・外出中・観察中）は、常時参照できるよう **右サイドバー「入院者情報」パネルに統合**。従来の病床占有・男女内訳・平均年齢に加えて、**稼働率・隔離中・拘束中・外出中・観察中**を同パネル内へ追記表示する。
