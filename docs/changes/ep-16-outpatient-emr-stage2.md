...
## 段階 2 完了前チェック — カルテ関連 ep 動作確認リスト（MASTER / 2026-05-08）

PM の「epでカルテが関係してる所一旦確認してほしい」要望を受け、段階 2 クローズ前に新カルテ画面に関わる全 ep の動線・機能を確認するためのチェックリスト。`http://localhost:5173/` （または :3000）で順次確認。

### ep-01 病棟マップ → カルテ遷移

- [ ] **`/` 病棟マップを開く** → ベッド一覧表示
- [ ] **ベッドをダブルクリック** → `/karte/<id>` に遷移（全患者で新カルテ・us-38 + 2026-05-08 統一）
- [ ] **遷移先で mode='inpatient'**（入院系 Chip / テーマ紺 / 看護過程タブ活性）
- [ ] **「一覧に戻る」リンク** → `/` に戻る

### ep-03 入退院指示

- [ ] 入院カルテのアクションバーに **「入院指示」「退院指示」** 2 ボタン表示（us-36 サブ A）
- [ ] `admissionState='inpatient'` の患者: 「入院指示」disabled + Tooltip / 「退院指示」活性
- [ ] `admissionState='outpatient'` の患者: 逆の活性状態
- [ ] 各ボタンクリックで既存 `AdmissionOrderDialog` / `DischargeOrderDialog` 起動

### ep-05〜08 隔離拘束系

- [ ] 入院カルテの **診療録タブ上部に `RestraintOrderLinks`**（隔離開始 / 解除 / 拘束開始 等の 6 リンク・us-36 サブ B）
- [ ] アクションバーの **「隔離拘束指示」** ボタン → `RestraintOrderDialog`（既定タイトル「隔離開始」）
- [ ] 診療録ヘッダリンク → `RestraintOrderDialog`（リンク別タイトルで開く）
- [ ] 外来 mode では `RestraintOrderLinks` 非表示

### ep-09 患者情報 → カルテ遷移

- [ ] **`/patients` 入院患者一覧** → 行クリックで `/patients/:id`（PatientMain・us-09）
- [ ] PatientMain の **「カルテを開く」ボタン** → `/karte/<id>` 入院 mode（昨日修正済）
- [ ] **`/patients` 直接ダブルクリック動線** → us-38 で `/karte/<id>` 入院 mode

### ep-10 看護実施・フローシート

- [ ] 入院 / 外来カルテのフローシートタブ → `FlowsheetPage embedded` が表示
- [ ] バイタルグラフ（VitalChart）が描画される（recharts）
- [ ] サイドバー「看護記録 / フローシート / 一括バイタル / 睡眠表 / 一括看護経過記録」が起動可能

### ep-12〜14 看護過程

- [ ] **us-37 完了後**: 入院カルテの看護過程タブ → `PatientCarePlan embedded` 等が表示
- [ ] 外来カルテでは看護過程タブ disabled（Tooltip「外来では利用しません」）
- [ ] サイドバー「看護過程」(`/care-plan`) は独立で起動可

### ep-15 段階 1（外来 EMR 刷新）

- [ ] **`/outpatient` 外来一覧** → 行クリックで `/karte/<id>` 外来 mode（テーマ緑）
- [ ] 看護過程タブ disabled
- [ ] AC-10 タブ切替で URL ハッシュ追従（`#medical-record` `#orders` 等）
- [ ] ブラウザバックで前タブに戻る
- [ ] `/karte-outpatient/:id` 直 URL → 404
- [ ] `/outpatient/:id/basic` 直 URL → `/karte/:id#patient-info` にリダイレクト

### ep-16 段階 2（本エピック・新規追加分）

- [ ] **us-44 患者ヘッダー強化**: 入院 / 外来バッジ強調 / 病棟・病室・ベッド併記 / 主治医・入院日・診断 / 8 ピクトグラム（アレルギー・感染症・妊娠・転倒・隔離拘束・嚥下・認知・要報告） / 隔離拘束バッジ
- [ ] **us-46 診療情報パネル**: 7 サブタブ（診断名 / 基本情報 / GAF / 院外文書 / ファミリ / クリニカルパス / 指示/入室）
- [ ] **us-45 生活歴タイムライン**: 5 行マトリクス（治療歴 / 訪問看護 / 学歴・経歴 / エピソード / 生活歴・現病歴）+ 空行表示トグル
- [ ] **us-43 診療録タブ**: 集約タイムライン + 新規記載ダイアログ
- [ ] **us-47 診療録追補**: 期間切替 Chip（6日 / 30日 / 全件）+ タグ filter Chip 7 種 + 「最初へ ▲」スクロール / 「続き ▼」snackbar
- [ ] **us-50 指示簿タブ**: ORDERS 一覧 / type Chip / status Chip / 指示状況タブを開くボタン
- [ ] **us-51 指示状況タブ**: ステータス別件数 / 期間切替 / 未対応マーカー / 指示簿タブを開くボタン
- [ ] **タブ配色**: バーが mode 色（入院紺・外来緑）、アクティブタブが白 + mode 色テキスト
- [ ] **タブアイコン**: 7 タブすべてアイコン付き
- [ ] **タブ内 SectionHeader 重複なし**（診療録・指示簿・指示状況・看護過程すべて）

### ep-16 段階 2（残）

- [ ] **us-36 サブ C（看護ケア記録）**: 要件未確定（実装方針 3 案から PM 判断・実装後ここに行追加）
- [ ] **us-52 スケジュールタブ**: 未着手（後発・実装後ここに行追加）

### 確認時の注意

- 各 ep の「カルテに関わる us」を全て新カルテ画面（`/karte/:patientId`）で実行できることが段階 2 のゴール
